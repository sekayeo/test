...
export default async function on({
  ...
  args: "Gunakan format: <packname>|<author>. Jika tidak ada |, author akan otomatis jadi nama kamu."
}, async ({
  media,
  args
}) => {
  ...
  let [packname, author] = args?.split("|").map(x => x.trim());
  if (!packname) return Exp.reply("⚠️ Packname tidak boleh kosong!");

  if (!author) author = cht.pushName;
